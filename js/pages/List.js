...
export default {
  ...
  <div class="filter-container" style="margin-bottom: 15px; display: flex; gap: 15px; padding: 5px 10px; align-items: center;">
    ...
    >
        ...
        <ul class="stats" v-if="list.findIndex(item => item[0].id === level.id) + 1 <= 50" style="margin-top: 15px;">
            <li>
            ...
            <li v-if="list.findIndex(item => item[0].id === level.id) + 1 <= 25">
                <div class="type-title-sm">GLCL score ({{ level.percentToQualify }}%)</div>
                <p>{{ (Number(score(list.findIndex(item => item[0].id === level.id) + 1, 100, level.percentToQualify)) / 10).toFixed(2) }}</p>
            </li>
        </ul>
    ...
    <div style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; margin-top: 20px;">
        ...
        >
            <h3 style="margin: 0; font-size: 1.2rem;">Position History (ordered by most recent)</h3>
            <span :style="{ 
